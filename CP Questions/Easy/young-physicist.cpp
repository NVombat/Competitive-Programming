...
the xi coordinate, the yi coordinate and the zi coordinate of the force vector, applied
...
int main()
{
    ...
    //sx sy sz represent the sum for each column x, y, z
    int sx=0, sy=0, sz=0;
    //x, y, z are arrays which will store the vector components for each x, y, z
    //We can do without the arrays and use variables in the loop as the question is to do with the sum
    //The vectors themselves don't matter
    int x[n], y[n], z[n];

    //Scan each component(x, y, z)
    for(int i=0; i<n; i++)
    {
        cin >> x[i] >> y[i] >> z[i];
        //Add it to its particular sum variable
        sx+=x[i];
        sy+=y[i];
        sz+=z[i];
        //cout << sx << "-->" << sy << "-->" << sz << endl;
    ...
    //Check if all sums add up to 0 -> If yes then print yes else no
